...
function toRecord(row: { id: string; assessment_type: "QUICKSCAN"; framework_version: "MIS-1.0"; engine_version: "ENGINE-1.0"; status: "IN_PROGRESS" | "SCORED" | "ARCHIVED"; created_at: string; updated_at: string; completed_at: string | null }, responses: MisResponse[], result?: MisScoringResult): AssessmentRecord {
  if (row.status === "ARCHIVED") throw new Error("ARCHIVED_ASSESSMENT");
  return { id: row.id, assessmentType: row.assessment_type, frameworkVersion: row.framework_version, engineVersion: row.engine_version, status: row.status, responses, result, createdAt: row.created_at, updatedAt: row.updated_at, completedAt: row.completed_at ?? undefined };
}

async function loadResponses(supabase: Awaited<ReturnType<typeof createSupabaseServerClient>>, assessmentId: string) {
  const { data, error } = await supabase.from("assessment_responses").select("question_id, score").eq("assessment_id", assessmentId);
...
async function loadResult(supabase: Awaited<ReturnType<typeof createSupabaseServerClient>>, assessmentId: string) {
  const { data, error } = await supabase.from("assessment_results").select("overall_score, maturity, dimension_scores, strengths, gaps, constraints, opportunities, roadmap").eq("assessment_id", assessmentId).maybeSingle();
  if (error) throw new Error(`ASSESSMENT_RESULT_READ_FAILED:${error.message}`);
  if (!data) return undefined;
  return { overallScore: Number(data.overall_score), maturity: data.maturity, dimensionScores: data.dimension_scores, strengths: data.strengths, gaps: data.gaps, constraints: data.constraints, opportunities: data.opportunities, roadmap: data.roadmap } as unknown as MisScoringResult;
}
...
export const supabaseAssessmentRepository: AssessmentRepository = {
  async createAssessment() {
    ...
    const { data, error } = await supabase.from("assessments").insert({ organization_id: organizationId, assessment_type: "QUICKSCAN", framework_version: "MIS-1.0", engine_version: "ENGINE-1.0", status: "IN_PROGRESS" }).select("id, assessment_type, framework_version, engine_version, status, created_at, updated_at, completed_at").single();
    if (error) throw new Error(`ASSESSMENT_CREATE_FAILED:${error.message}`);
    return toRecord(data, []);
  },

  async getAssessment(id) {
    const supabase = await createSupabaseServerClient();
    const { data, error } = await supabase.from("assessments").select("id, assessment_type, framework_version, engine_version, status, created_at, updated_at, completed_at").eq("id", id).maybeSingle();
  ...
  async saveResponse(assessmentId, questionId, score) {
    ...
    const { data: assessment, error: assessmentError } = await supabase.from("assessments").select("id, assessment_type, framework_version, engine_version, status, created_at, updated_at, completed_at").eq("id", assessmentId).maybeSingle();
  ...
  async completeAssessment(id) {
    const supabase = await createSupabaseServerClient();
    const { data: assessment, error } = await supabase.from("assessments").select("id, assessment_type, framework_version, engine_version, status, created_at, updated_at, completed_at").eq("id", id).maybeSingle();
    if (error) throw new Error(`ASSESSMENT_READ_FAILED:${error.message}`);
    if (!assessment) throw new Error("ASSESSMENT_NOT_FOUND");
    ...
    const { data: updated, error: updateError } = await supabase.from("assessments").update({ status: "SCORED", completed_at: now, updated_at: now }).eq("id", id).eq("status", "IN_PROGRESS").select("id, assessment_type, framework_version, engine_version, status, created_at, updated_at, completed_at").maybeSingle();
