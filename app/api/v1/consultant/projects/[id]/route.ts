...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await supabase.from<Record<string, unknown>>("consulting_projects").select(projectColumns).eq("id", id).eq("organization_id", context.organizationId).maybeSingle();
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.status !== undefined && (typeof body.status !== "string" || !statuses.has(body.status.toUpperCase()))) return jsonError("Invalid project status.", 400, "INVALID_PROJECT_STATUS");
    for (const [field, value] of [["startDate", body.startDate], ["targetEndDate", body.targetEndDate]] as const) {
      if (value !== undefined && value !== null && (typeof value !== "string" || !isoDate.test(value))) return jsonError(`${field} must use YYYY-MM-DD format or null.`, 400, "INVALID_PROJECT_DATE");
    }
    if (body.completedAt !== undefined && body.completedAt !== null && (typeof body.completedAt !== "string" || Number.isNaN(Date.parse(body.completedAt)))) return jsonError("completedAt must be a valid timestamp or null.", 400, "INVALID_COMPLETED_AT");
    const supabase = await createSupabaseServerClient();
    const current = await supabase.from<Record<string, unknown>>("consulting_projects").select("id, organization_id, status, start_date, target_end_date, completed_at").eq("id", id).eq("organization_id", context.organizationId).maybeSingle();
    ...
    if (!transitions[currentStatus]?.has(nextStatus)) return jsonError(`Invalid project status transition: ${currentStatus} -> ${nextStatus}.`, 409, "INVALID_PROJECT_STATUS_TRANSITION");
    ...
    if (nextStatus === "COMPLETED" && payload.completed_at === null) return jsonError("completedAt cannot be null when completing a project.", 400, "COMPLETED_AT_REQUIRED");
    if (nextStatus === "COMPLETED" && payload.completed_at === undefined && current.data.completed_at == null) payload.completed_at = new Date().toISOString();
    if (nextStatus !== "COMPLETED" && payload.completed_at !== undefined && payload.completed_at !== null) return jsonError("completedAt requires COMPLETED status.", 400, "COMPLETED_AT_REQUIRES_COMPLETED_STATUS");
    const updated = await supabase.from<Record<string, unknown>>("consulting_projects").update(payload).eq("id", id).eq("organization_id", context.organizationId).select(projectColumns).single();
