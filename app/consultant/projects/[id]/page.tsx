...
type Project = { id: string; opportunity_id: string; name: string; status: string; start_date: string | null; target_end_date: string | null; completed_at: string | null; contract_value: number | null; currency: string | null };
type ValueCase = { investment: number; expected_annual_benefit: number; actual_annual_benefit: number | null; currency: string; roi_percent: number; payback_months: number | null; actual_roi_percent: number | null; actual_payback_months: number | null };

export default async function ConsultantProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const context = await getOrganizationContext();
  const supabase = await createSupabaseServerClient();
  const projectResult = await supabase.from<Record<string, unknown>>("consulting_projects").select("id, opportunity_id, name, status, start_date, target_end_date, completed_at, contract_value, currency").eq("id", id).eq("organization_id", context.organizationId).maybeSingle();
  if (projectResult.error || !projectResult.data) notFound();
  const project = projectResult.data as Project;
  const valueResult = await supabase.from<Record<string, unknown>>("opportunity_value_cases").select("investment, expected_annual_benefit, actual_annual_benefit, currency, roi_percent, payback_months, actual_roi_percent, actual_payback_months").eq("organization_id", context.organizationId).eq("opportunity_id", project.opportunity_id).maybeSingle();
