...
export default async function ConsultantProjectsPage() {
  let projects = await listConsultingProjects();
  let valueCases = await listValueCases();
      ...
      <div className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          {projects.length === 0 ? <div className="px-5 py-10 text-sm text-slate-500">No consulting projects yet. Move an opportunity to WON in the pipeline to create an engagement.</div> : projects.map((project) => {
            ...
            return <div key={project.id} className="grid grid-cols-6 gap-4 border-b border-slate-100 px-5 py-4 text-sm last:border-0"><div><p className="font-medium">{project.name}</p><p className="mt-1 text-xs text-slate-500">{project.id.slice(0, 8)}</p></div><span className="text-xs font-semibold">{project.status}</span><span>{project.startDate ?? "—"}</span><span>{project.targetEndDate ?? "—"}</span><span>{project.contractValue == null ? "—" : `${project.currency ?? "USD"} ${project.contractValue.toLocaleString()}`}</span><span>{value?.actualRoiPercent == null ? "Not measured" : `${value.actualRoiPercent.toFixed(1)}% ROI`}</span></div>;
          })}
        </div>
      </div>
    </main>
