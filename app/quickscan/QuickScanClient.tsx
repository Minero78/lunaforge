    ...
    return (
      <main className="min-h-screen bg-slate-950 px-6 py-16 text-white">
        <div className="mx-auto max-w-3xl">
          <p className="mb-4 text-sm font-semibold uppercase tracking-[0.24em] text-sky-300">Stratova</p>
          <h1 className="max-w-2xl text-4xl font-semibold tracking-tight sm:text-6xl">Mining Intelligence QuickScan™</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">Discover how ready your organization is for data, digital transformation, spatial intelligence, and AI.</p>
          <button type="button" onClick={startAssessment} disabled={loading} className="mt-10 rounded-full bg-white px-7 py-3 font-semibold text-slate-950 transition hover:bg-slate-200 disabled:cursor-not-allowed disabled:opacity-60">
      ...
      <div className="mx-auto max-w-3xl">
        <div className="mb-10 flex items-center justify-between gap-6">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">Question {currentIndex + 1} of {MIS_QUICKSCAN_QUESTIONS.length}</p>
          </div>
        ...
        <section className="mt-12 rounded-3xl border border-white/10 bg-white/5 p-7 sm:p-10">
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-5">
            {SCORE_OPTIONS.map((score) => (
              <button key={score} type="button" onClick={() => saveAnswer(score)} disabled={loading} className={`rounded-2xl border px-4 py-5 text-left transition ${currentAnswer === score ? "border-sky-300 bg-sky-300/15 text-white" : "border-white/10 bg-white/5 text-slate-300 hover:border-white/30"} disabled:cursor-not-allowed disabled:opacity-60`}>
                <span className="block text-2xl font-semibold">{score}</span>
                <span className="mt-1 block text-xs text-slate-400">{score === 1 ? "Very low" : score === 5 ? "Very high" : ""}</span>
              </button>
            ))}
          </div>
        </section>
        <div className="mt-8 flex items-center justify-between gap-4">
          <button type="button" onClick={() => setCurrentIndex((index) => Math.max(0, index - 1))} disabled={currentIndex === 0 || loading} className="rounded-full border border-white/10 px-6 py-3 font-medium text-slate-300 transition hover:border-white/30 disabled:cursor-not-allowed disabled:opacity-40">Back</button>
          <button type="button" onClick={next} disabled={!currentAnswer || loading} className="rounded-full bg-white px-7 py-3 font-semibold text-slate-950 transition hover:bg-slate-200 disabled:cursor-not-allowed disabled:opacity-40">
