      ...
      <div className="mx-auto max-w-6xl">
        <header>
          ...
          <h1 className="mt-4 max-w-4xl text-4xl font-semibold tracking-tight sm:text-6xl">
            Your Mining Intelligence Profile
          </h1>
          <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-300">
            A decision-oriented view of your organization&apos;s current capabilities, constraints, and next opportunity.
          </p>
        </header>
        ...
        <section className="mt-10 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-3xl border border-white/10 bg-white/5 p-8 sm:p-10">
            ...
            <div className="mt-4 flex items-end gap-4">
              <span className="text-7xl font-semibold tracking-tight">{data.overallScore}</span>
              <span className="pb-3 text-slate-400">/ 100</span>
            </div>
            <p className="mt-4 inline-flex rounded-full border border-sky-300/20 bg-sky-300/10 px-4 py-2 text-sm font-semibold text-sky-200">
              {data.maturity}
            </p>
          </div>
          <div className="rounded-3xl border border-white/10 bg-white/5 p-8 sm:p-10">
        ...
        <section className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-8 sm:p-10">
          <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
            <div>
              <p className="text-sm uppercase tracking-wider text-slate-400">Capability profile</p>
              <h2 className="mt-2 text-2xl font-semibold">Where the organization stands</h2>
            </div>
            ...
            {data.dimensionScores.map((item) => (
              <div key={item.dimension}>
                <div className="mb-2 flex justify-between gap-4 text-sm">
                  <span className="text-slate-200">{DIMENSION_LABELS[item.dimension]}</span>
                  <span className="font-semibold text-white">{item.score}</span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-white/10">
                  <div className="h-full rounded-full bg-sky-400" style={{ width: `${item.score}%` }} />
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mt-8 grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-3xl border border-sky-300/20 bg-sky-300/5 p-8">
            ...
            <div className="mt-6 flex flex-wrap gap-2 text-xs font-semibold">
              <span className="rounded-full bg-white/10 px-3 py-2">Impact: {diagnosis.opportunity.impact}</span>
              <span className="rounded-full bg-white/10 px-3 py-2">Effort: {diagnosis.opportunity.effort}</span>
              <span className="rounded-full bg-white/10 px-3 py-2">Readiness: {diagnosis.opportunity.readiness}</span>
            </div>
          </div>
        </section>
        ...
        <section className="mt-8 rounded-3xl border border-white/10 bg-white/5 p-8 sm:p-10">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {diagnosis.recommendations.map((recommendation, index) => (
              <article key={recommendation.title} className="rounded-2xl border border-white/10 bg-black/10 p-6">
                <p className="text-xs font-semibold uppercase tracking-wider text-sky-300">0{index + 1} · {recommendation.horizon}</p>
        ...
        <footer className="py-10 text-xs text-slate-500">
          Assessment {data.assessmentId} · Results generated from the deterministic MIS 1.0 engine.
        </footer>
      </div>
    </main>
  );
